...
class PlayerStateTracker:
    ...
    def __init__(self, player_index: int, metadata: Optional[dict] = None, config_path: Optional[Path] = None):
        ...
        self.current_state = {
            ...
            # Phase 1 extensions
            "weapon": None,        # Weapon name string or None
            "weapon_tier": None,   # WeaponTier string or None
            "credits": None,       # Integer credits or None
            "pos_x": None,         # Normalized map x [0,1] or None
            "pos_y": None,         # Normalized map y [0,1] or None
            # Cross-round accumulators (never reset mid-map)
            "match_kills": 0,
            "match_deaths": 0,
        }
    ...
    def update(self, detections: dict, timestamp: float) -> None:
        ...
        # Update weapon (WeaponInfo)
        weapon_info = detections.get("weapon")
        if weapon_info is not None:
            self.current_state["weapon"] = weapon_info.weapon_name
            self.current_state["weapon_tier"] = weapon_info.weapon_tier.value

        # Update credits (integer from OCR)
        credits_val = detections.get("credits")
        if credits_val is not None:
            self.current_state["credits"] = credits_val

        # Update minimap position (PlayerPosition)
        position = detections.get("position")
        if position is not None:
            self.current_state["pos_x"] = round(position.x, 4)
            self.current_state["pos_y"] = round(position.y, 4)
        elif detections.get("position_missing"):
            # Explicitly marked as not detected this frame
            self.current_state["pos_x"] = None
            self.current_state["pos_y"] = None
    ...
    def reset_for_new_round(self) -> None:
        ...
        self.current_state = {
            ...
            "pos_x": None,
            "pos_y": None,
            # Preserve cross-round kill/death tallies
            "match_kills": self.current_state.get("match_kills", 0),
            "match_deaths": self.current_state.get("match_deaths", 0),
        }
        self.previous_state = self.current_state.copy()
        self.revival_candidate_count = 0
        self.revival_first_detection_timestamp = None
