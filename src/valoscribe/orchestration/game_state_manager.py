...
class GameStateManager:
    ...
    def __init__(
        ...
        # Agent identity seeding (template matching at round start, retried each frame
        # until all alive players have a position or IDENTITY_SEED_MAX_FRAMES is reached)
        self._minimap_identity_seeded = False
        self._identity_seed_frames: int = 0

        # Masked minimap image for current frame (set in _process_active_round, None otherwise)
        self._current_minimap_image: Optional[np.ndarray] = None

        # Last computed timers dict — cached so FramePredictor can read them without re-detecting
        self._last_timers: Optional[dict] = None

        # Player trackers - initialized lazily on first preround
        self.player_trackers: Optional[list[PlayerStateTracker]] = None

        # Optional live round win predictor (set externally before processing)
    ...
    def _init_components(self, config_path: Optional[Path]) -> None:
        ...
        map_name = self.vlr_metadata.get("map", "").lower().strip() or None
        self.detector_registry = DetectorRegistry(config_path=config_path, map_name=map_name)
    ...
    def process_frame(self, timestamp: float, frame: np.ndarray) -> None:
        ...
        # 4. Live round win prediction (only if FramePredictor is configured)
        if self.frame_predictor is not None:
            prob = self.frame_predictor.predict_from_state(self, timestamp)
            if prob is not None:
                self._emit_prediction(prob, timestamp)

        # 5. Progress feedback (every 100 frames)
    ...
    def _handle_phase_transition(
        ...
        if new_phase == Phase.PREROUND and (old_phase == Phase.POST_ROUND or old_phase == Phase.ACTIVE_ROUND or old_phase is None):
            ...
            # Reset player states, timers, state validator, and minimap tracks for new round
    ...
    def _process_preround(self, timestamp: float, frame: np.ndarray) -> None:
        ...
        self._current_minimap_image = None
    ...
    def _process_active_round(
        ...
        # Determine current sides for position attribution.
        current_sides = self.round_manager.get_current_sides()

        # Capture masked minimap for model input.
        self._current_minimap_image = self.detector_registry.minimap_detector.get_masked_minimap(frame)

        # Update each alive player's position via per-player icon template matching.
        # Dead players (killfeed-confirmed) are automatically excluded from the search.
        self._update_player_positions_from_map(frame, current_sides)
        ...
        # Pre-detect all 10 agent slots ONCE and share across all player lookups.
        # Without this, each player searches its 5 slots independently, causing
        # each slot to be template-matched up to 5x per frame.
        color_agents = [
            self.detector_registry.inround_agent_detector.detect(frame, i)
            for i in range(10)
        ]
        grey_agents: list | None = None  # computed lazily only if needed

        # Detect states by searching for each player's agent icon
        for player_tracker in self.player_trackers:
            # Skip dead players (stay dead until revival); allow grace period at round start
            ...
            # Search for this player's agent icon using pre-detected slot cache
            found_slot = self._find_player_agent_slot_cached(player_tracker, color_agents)
            if found_slot is None and player_tracker.frames_icon_missing > 0:
                # Only run greyscale pass (dead-player UI) after icon has been
                # missing at least one frame — avoids cost on every clean frame
                if grey_agents is None:
                    grey_agents = [
                        self.detector_registry.inround_agent_detector.detect(frame, i, greyscale=True)
                        for i in range(10)
                    ]
                found_slot = self._find_player_agent_slot_cached(player_tracker, grey_agents)
            ...
            if found_slot is not None:
                ...
                # Detect health (every frame — needed for death detection)
                health = self.detector_registry.health_detector.detect(frame, found_slot, side)
                detections["health"] = health

                # Detect armor (every frame — changes with damage)
                armor = self.detector_registry.armor_detector.detect(frame, found_slot, side)
                if armor is not None:
                    detections["armor"] = armor

                # Weapon, abilities, and ultimate change slowly: run at 1fps (every 4th frame).
                if self.frame_count % 4 == 0:
                    weapon_info = self.detector_registry.weapon_detector.detect(frame, found_slot, side)
                    if weapon_info is not None:
                        detections["weapon"] = weapon_info
        ...
        for kill_detection in valid_kills:
            ...
            if killer_player:
                killer_player.current_state["match_kills"] = killer_player.current_state.get("match_kills", 0) + 1
    ...
    def _process_post_round(
        ...
        # Pre-detect all 10 slots once (same cache pattern as _process_active_round)
        post_color_agents = [
            self.detector_registry.inround_agent_detector.detect(frame, i)
            for i in range(10)
        ]
        ...
        for player_tracker in self.player_trackers:
            ...
            found_slot = self._find_player_agent_slot_cached(player_tracker, post_color_agents)
    ...
    def _update_player_positions_from_map(
        self, frame: np.ndarray, current_sides: dict
    ) -> None:
        """
        Per-player minimap position update using direct icon template matching.

        For each alive player, searches the full masked minimap for their agent-specific
        icon (side-colored ring + portrait).  Players confirmed dead from the killfeed
        have their position cleared immediately and are not searched.

        Nearest-neighbour fallback: if a player's icon cannot be found this frame
        (smoke, occlusion, score below threshold), their last-known position is kept
        and velocity is decayed.  This prevents a single bad frame from losing track.
        """
        if self.player_trackers is None:
            return

        roster: list[tuple[str, str]] = []
        tracker_by_key: dict[tuple[str, str], object] = {}

        for tracker in self.player_trackers:
            if not tracker.current_state.get("alive", True):
                tracker.current_state["pos_x"] = None
                tracker.current_state["pos_y"] = None
                continue
            agent = tracker.metadata.get("agent")
            if not agent:
                continue
            team_name = tracker.metadata.get("team")
            team_key = ("team1" if team_name == self.round_manager.team_names[0]
                        else "team2" if team_name == self.round_manager.team_names[1]
                        else None)
            if not team_key:
                continue
            side = current_sides[team_key]
            key = (agent, side)
            roster.append(key)
        ...
        located = self.detector_registry.minimap_detector.locate_players(frame, roster)

        _VEL_ALPHA = 0.4
        for key in roster:
            tracker = tracker_by_key[key]
            result = located.get(key)

            if result is not None:
                nx, ny, conf = result
                px = tracker.current_state.get("pos_x")
                py = tracker.current_state.get("pos_y")
                if px is not None and py is not None:
                    tracker.current_state["pos_vx"] = (
                        _VEL_ALPHA * (nx - px)
                        + (1 - _VEL_ALPHA) * tracker.current_state.get("pos_vx", 0.0)
                    )
                    tracker.current_state["pos_vy"] = (
                        _VEL_ALPHA * (ny - py)
                        + (1 - _VEL_ALPHA) * tracker.current_state.get("pos_vy", 0.0)
                    )
                else:
                    tracker.current_state["pos_vx"] = 0.0
                    tracker.current_state["pos_vy"] = 0.0
                tracker.current_state["pos_x"] = round(nx, 4)
                tracker.current_state["pos_y"] = round(ny, 4)
                log.debug(
                    f"[POS] {tracker.metadata.get('name')} ({key[0]}/{key[1]}) "
                    f"-> ({nx:.3f},{ny:.3f}) conf={conf:.3f}"
                )
            else:
                # Not found this frame — decay velocity, keep last known position
                tracker.current_state["pos_vx"] = tracker.current_state.get("pos_vx", 0.0) * 0.5
                tracker.current_state["pos_vy"] = tracker.current_state.get("pos_vy", 0.0) * 0.5
                log.debug(
                    f"[POS] {tracker.metadata.get('name')} ({key[0]}/{key[1]}) "
                    f"not located — keeping last known position"
                )

    def _seed_minimap_identity(self, frame: np.ndarray, current_sides: dict) -> None:
        """
        Use agent portrait template matching to seed round-start minimap positions.

        Called once per round on the first ACTIVE_ROUND frame. Seeds pos_x/pos_y on each
        player tracker so nearest-neighbour tracking has a correct anchor from frame 1.
        Players whose template is not found confidently remain at pos_x=None and get
        assigned to leftover blobs by _distribute_minimap_positions (pass 2 fallback).
        """
        if self.player_trackers is None:
            return

        # Build roster: (agent_name, side) for every ALIVE player only.
        # Dead players are known individually from the killfeed — no need to search
        # for their icon on the minimap, and including them pollutes the assignment.
        roster = []
        for tracker in self.player_trackers:
            if not tracker.current_state.get("alive", True):
                continue
            ...
            if not team_key:
                continue
            side = current_sides[team_key]
            roster.append((agent, side))

        if not roster:
            return

        agent_positions = self.detector_registry.minimap_detector.identify_agents(frame, roster)

        if not agent_positions:
            return

        # Seed each tracker's position from the identified (agent, side) match.
        # Skip dead players and trackers already seeded (multi-frame retry — only fill gaps).
        for tracker in self.player_trackers:
            if not tracker.current_state.get("alive", True):
                continue  # dead — killfeed already told us, no icon to find
            if tracker.current_state.get("pos_x") is not None:
                continue  # already seeded on an earlier frame this round
            agent = tracker.metadata.get("agent")
            team_name = tracker.metadata.get("team")
            team_key = ("team1" if team_name == self.round_manager.team_names[0]
                        else "team2" if team_name == self.round_manager.team_names[1]
            ...
            if not team_key:
                continue
            side = current_sides[team_key]
            pos = agent_positions.get((agent, side))
            if pos is not None:
                tracker.current_state["pos_x"] = pos[0]
                tracker.current_state["pos_y"] = pos[1]
                tracker.current_state["pos_vx"] = 0.0
                tracker.current_state["pos_vy"] = 0.0
                log.debug(
                    f"[IDENTITY] {tracker.metadata.get('name')} ({agent}/{side}) seeded at "
                    f"({pos[0]:.3f}, {pos[1]:.3f}) (seed_frame={self._identity_seed_frames})"
                )
            else:
                log.debug(
                    f"[IDENTITY] {tracker.metadata.get('name')} ({agent}/{side}) not identified "
                    f"— will use leftover blob assignment"
                )

    def _distribute_minimap_positions(self, minimap_positions, current_sides: dict) -> None:
        """
        Assign raw minimap blobs to specific player trackers via nearest-neighbour.

        MinimapDetector returns raw blobs (sorted by circularity, no identity).
        Here we match each blob to the alive tracker whose last-known position is
        closest, preserving per-player identity across frames.

        Dead players have their position cleared to None.
        Trackers with no prior position (first frame of a round) receive leftover blobs.
        """
        if self.player_trackers is None:
            return
        ...
        # Separate alive trackers by current side; clear dead players immediately
        atk_alive, def_alive = [], []
        for tracker in self.player_trackers:
            team_name = tracker.metadata.get("team")
            team_key = ("team1" if team_name == self.round_manager.team_names[0]
            ...
            if not team_key:
                continue
            side = current_sides[team_key]
            if tracker.current_state.get("alive", True):
                (atk_alive if side == "attack" else def_alive).append(tracker)
            else:
                tracker.current_state["pos_x"] = None
                tracker.current_state["pos_y"] = None

        self._match_blobs_to_trackers(
            list(minimap_positions.attack_positions), atk_alive,
            MinimapDetector.TRACK_MAX_DIST,
        )
        self._match_blobs_to_trackers(
            list(minimap_positions.defense_positions), def_alive,
            MinimapDetector.TRACK_MAX_DIST,
        )

    @staticmethod
    def _match_blobs_to_trackers(blobs, trackers, max_dist: float) -> None:
        """
        Hungarian-algorithm assignment: match blobs to trackers by minimising total
        distance, using each tracker's velocity-predicted next position as the anchor.

        This replaces greedy nearest-neighbour, which caused identity swaps whenever
        two players crossed paths (the first tracker processed would steal the blob).
        The globally optimal assignment avoids this entirely.

        Velocity is tracked as an EMA of the frame-over-frame displacement.  Predicting
        `pos + vel` as the expected next position handles fast-moving agents (Jett dash,
        Chamber TP) that would otherwise jump past max_dist in a single frame.

        Pass 2: trackers with no known position receive leftover blobs (first-frame or
        just-respawned players that weren't seeded by identity matching).
        """
        from scipy.optimize import linear_sum_assignment

        all_blobs = list(blobs)
        known   = [(t, t.current_state.get("pos_x"), t.current_state.get("pos_y"))
                   for t in trackers if t.current_state.get("pos_x") is not None]
        unknown = [t for t in trackers if t.current_state.get("pos_x") is None]

        if known and all_blobs:
            n_t, n_b = len(known), len(all_blobs)
            # Cost matrix: predicted position → blob (Euclidean, normalised coords)
            cost = np.empty((n_t, n_b), dtype=np.float32)
            for i, (t, px, py) in enumerate(known):
                pred_x = px + t.current_state.get("pos_vx", 0.0)
                pred_y = py + t.current_state.get("pos_vy", 0.0)
                for j, b in enumerate(all_blobs):
                    cost[i, j] = ((b.x - pred_x) ** 2 + (b.y - pred_y) ** 2) ** 0.5
            ...
            for ri, ci in zip(row_ind, col_ind):
                ...
                if cost[ri, ci] <= max_dist:
                    blob = all_blobs[ci]
                    # Update EMA velocity from actual displacement
                    _VEL_ALPHA = 0.4
                    vx_new = blob.x - px
                    vy_new = blob.y - py
                    t.current_state["pos_vx"] = (
                        _VEL_ALPHA * vx_new
                        + (1 - _VEL_ALPHA) * t.current_state.get("pos_vx", 0.0)
                    )
                    t.current_state["pos_vy"] = (
                        _VEL_ALPHA * vy_new
                        + (1 - _VEL_ALPHA) * t.current_state.get("pos_vy", 0.0)
                    )
                    t.current_state["pos_x"] = round(blob.x, 4)
                    t.current_state["pos_y"] = round(blob.y, 4)
                    assigned_trackers.add(ri)
                    assigned_blobs.add(ci)

            # Clear trackers not matched (decay velocity — player may be stationary offscreen)
            for i, (t, _, _) in enumerate(known):
                if i not in assigned_trackers:
                    t.current_state["pos_vx"] = t.current_state.get("pos_vx", 0.0) * 0.5
                    t.current_state["pos_vy"] = t.current_state.get("pos_vy", 0.0) * 0.5
                    t.current_state["pos_x"] = None
                    t.current_state["pos_y"] = None

            remaining = [b for i, b in enumerate(all_blobs) if i not in assigned_blobs]
        else:
            # No known trackers or no blobs — clear all known and keep all blobs as leftover
            for t, _, _ in known:
                t.current_state["pos_vx"] = t.current_state.get("pos_vx", 0.0) * 0.5
                t.current_state["pos_vy"] = t.current_state.get("pos_vy", 0.0) * 0.5
                t.current_state["pos_x"] = None
        ...
        # Pass 2: trackers with no prior position get leftover blobs (sorted by circularity)
        for tracker in unknown:
            if remaining:
                blob = remaining.pop(0)
                tracker.current_state["pos_x"] = round(blob.x, 4)
                tracker.current_state["pos_y"] = round(blob.y, 4)
                tracker.current_state["pos_vx"] = 0.0
                tracker.current_state["pos_vy"] = 0.0
            else:
                tracker.current_state["pos_x"] = None
    ...
    def _find_player_agent_slot_cached(self, player_tracker, agents_cache: list) -> Optional[int]:
        """
        Look up which slot contains a player's agent icon from a pre-computed
        per-slot cache (avoids redundant template matching across players).

        Args:
            player_tracker: PlayerStateTracker to search for
            agents_cache: List of length 10 where agents_cache[i] is the agent
                          string detected in slot i (or None if not detected).
                          Produced by detect(frame, i) for i in range(10).

        Returns:
            Slot index (0-9) if found, None otherwise.
        """
        player_agent = player_tracker.metadata.get("agent")
        if not player_agent:
            return None

        slots = range(0, 5) if player_tracker.player_index < 5 else range(5, 10)
        for slot_idx in slots:
            if agents_cache[slot_idx] == player_agent:
                return slot_idx
        return None
    ...
    def _mark_victim_as_killed(self, kill_detection) -> None:
        ...
        if victim_player:
            ...
            victim_player.current_state["alive"] = False
            victim_player.current_state["health"] = None
            victim_player.current_state["match_deaths"] = victim_player.current_state.get("match_deaths", 0) + 1
            log.info(
    ...
    def _write_frame_state(self, timestamp: float, frame: np.ndarray) -> None:
        ...
        # Cache timers so FramePredictor can read them on the same frame
        self._last_timers = timers

        # Write to CSV (and save minimap image if captured this frame)
        self.output_writer.write_frame_state(
            ...
            timers=timers,
            minimap_image=self._current_minimap_image,
        )
    ...
    def _emit_prediction(self, prob: float, timestamp: float) -> None:
        """Print a live round win probability line to stdout."""
        rm = self.round_manager
        timers = self._last_timers or {}
        game_timer = timers.get("game_timer") or 0.0
        spike_timer = timers.get("spike_timer")

        spike_str = f"  SPIKE PLANTED ({spike_timer:.0f}s)" if spike_timer else ""
        team1_name = rm.team_names[0]
        team2_name = rm.team_names[1]
        print(
            f"[round={rm.current_round:2d}  timer={game_timer:.0f}s{spike_str}]"
            f"  ATK {prob:.1%}  DEF {1-prob:.1%}"
            f"  ({team1_name} {rm.current_score['team1']}-{rm.current_score['team2']} {team2_name})"
        )
