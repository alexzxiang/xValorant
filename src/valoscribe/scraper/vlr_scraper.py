...
class VLRScraper:
    ...
    def scrape_event_stats(self, event_url: str) -> dict:
        """
        Scrape the VLR.gg event stats page for all player performance stats.

        URL formats accepted:
            https://www.vlr.gg/event/stats/2765/valorant-masters-london-2026
            https://www.vlr.gg/event/stats/2765          (slug optional)
            2765                                          (bare event ID)

        Returns a dict keyed by lowercase player handle:
        {
            "primmie": {
                "vlr_id": 25255,
                "team": "FS",
        ...
        """
        # Normalise input to a full URL
        url = event_url.strip()
        if url.isdigit():
            url = f"https://www.vlr.gg/event/stats/{url}"
        elif not url.startswith("http"):
            url = f"https://www.vlr.gg/event/stats/{url}"
        # Ensure /stats/ is in the path
        if "/event/" in url and "/stats/" not in url:
            url = url.rstrip("/") + "/stats"

        log.info(f"Scraping event stats from {url}")
        response = self.session.get(url, timeout=self.timeout)
        response.raise_for_status()

        soup = BeautifulSoup(response.text, "html.parser")
        table = soup.find("table", class_=lambda c: c and "mod-stats" in c)
        if not table:
            raise ValueError(f"No stats table found at {url}")

        tbody = table.find("tbody")
        if not tbody:
            raise ValueError("Stats table has no tbody")
        ...
        for row in tbody.find_all("tr"):
            ...
            if not link:
                continue
            href = link["href"]                   # e.g. /player/25255/primmie
            parts = href.strip("/").split("/")
            vlr_id = int(parts[1]) if len(parts) >= 3 and parts[1].isdigit() else 0
            name_div = link.find("div", style=lambda s: s and "font-weight: 700" in s)
            player_name = (name_div.get_text(strip=True) if name_div else link.get_text(strip=True)).lower()

            # Team abbreviation (mislabelled as "stats-player-country")
            ...
            agents_cell = cells[1]
            agents = [
                img["src"].split("/")[-1].replace(".png", "").lower()
                for img in agents_cell.find_all("img")
                if img.get("src")
            ]

            # Numeric columns in order: Rnd, R2.0, ACS, K:D, KAST, ADR, KPR, APR, FKPR, FDPR, HS%, CL%, CL, ...
            vals = [c.get_text(strip=True) for c in cells[2:]]

            # CL format: "1/18"  →  won, total
            cl_str = vals[11] if len(vals) > 11 else "0/0"
            cl_parts = cl_str.split("/")
            cl_won   = int(cl_parts[0]) if cl_parts[0].isdigit() else 0
            cl_total = int(cl_parts[1]) if len(cl_parts) > 1 and cl_parts[1].isdigit() else 0

            results[player_name] = {
                "vlr_id":    vlr_id,
                "team":      team,
                "rounds":    _int(vals[0])   if len(vals) > 0  else 0,
                "rating":    _float(vals[1]) if len(vals) > 1  else 0.0,
                "acs":       _float(vals[2]) if len(vals) > 2  else 0.0,
                "kd":        _float(vals[3]) if len(vals) > 3  else 0.0,
                "kast":      _pct(vals[4])   if len(vals) > 4  else 0.0,
                "adr":       _float(vals[5]) if len(vals) > 5  else 0.0,
                "kpr":       _float(vals[6]) if len(vals) > 6  else 0.0,
                "apr":       _float(vals[7]) if len(vals) > 7  else 0.0,
                "fkpr":      _float(vals[8]) if len(vals) > 8  else 0.0,
                "fdpr":      _float(vals[9]) if len(vals) > 9  else 0.0,
                "hs_pct":    _pct(vals[10])  if len(vals) > 10 else 0.0,
                "cl_pct":    _pct(vals[11])  if len(vals) > 11 else 0.0,
                "cl_won":    cl_won,
                "cl_total":  cl_total,
                "agents":    agents,
            }
            log.debug(f"  {player_name} ({team}): rating={results[player_name]['rating']}, acs={results[player_name]['acs']}")

        log.info(f"Scraped stats for {len(results)} players")
        return results
