...
def process_vod(
    ...
    target_height: Optional[int] = typer.Option(
        None,
        "--target-height",
        help="Resize each frame to this height before processing (e.g. 1080 for 720p VODs). "
    ...
    predict_model: Optional[Path] = typer.Option(
        None,
        "--predict-model",
        help="Model directory (e.g. models/masters_london). Enables live round win probability output.",
    ),
    predict_map: Optional[str] = typer.Option(
        None,
        "--map",
        help="Map name for model routing (e.g. 'fracture'). Defaults to map in metadata.json.",
    ),
):
    """
    ...
    # Attach live round win predictor if requested
    if predict_model is not None:
        from valoscribe.inference.frame_predictor import FramePredictor
        map_name = (predict_map or vlr_metadata.get("map", "")).lower().strip()
        if not map_name:
            typer.echo("Error: --map is required when --predict-model is set (or set map in metadata.json)", err=True)
            raise typer.Exit(1)
        manager.frame_predictor = FramePredictor(
            model_dir=predict_model,
            map_name=map_name,
        )
