...
class VideoReader:
    ...
    def __next__(self) -> FrameInfo:
        ...
        while True:
            ...
            # Upscale to target height if needed (e.g. 720p → 1080p)
            if self.target_height is not None and frame.shape[0] != self.target_height:
                scale = self.target_height / frame.shape[0]
                new_w = round(frame.shape[1] * scale)
                frame = cv2.resize(frame, (new_w, self.target_height), interpolation=cv2.INTER_LINEAR)
